      ...
      for (u, u_paths) in paths.items():
          for (v, u_v_paths) in u_paths.items():
              for (w, v_w_paths) in paths[v].items():
                  ...
                  u_paths[w][s] += count
      print(sum(paths['svr']['dac'].values()) * sum(paths['dac']['fft'].values()) * sum(paths['fft']['out'].values()) +
            sum(paths['svr']['fft'].values()) * sum(paths['fft']['dac'].values()) * sum(paths['dac']['out'].values()))
